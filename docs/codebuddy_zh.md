...
在 [CodeBuddy](https://codebuddy.ai/) 中使用 planning-with-files。

---

## 安装

CodeBuddy 会自动从 `.codebuddy/skills/` 目录发现技能。提供两种安装方法：
...
# 将 CodeBuddy 技能复制到你的仓库
cp -r /tmp/planning-with-files/.codebuddy .

# 提交以与团队共享
git add .codebuddy/
git commit -m "为 CodeBuddy 添加 planning-with-files 技能"
git push
...
现在，你团队中使用 CodeBuddy 的每个人都可以访问这个技能！
...
重启你的 CodeBuddy 会话，然后当你处理复杂任务时，技能将自动激活。

无需斜杠命令——CodeBuddy 会根据你的任务描述自动调用技能！
...
CodeBuddy 扫描你的任务，并在以下情况时自动激活该技能：
- 提及“复杂任务”或“多步骤项目”
- 请求规划或组织
- 开始研究任务
- 处理需要超过 5 个步骤的项目
...
CodeBuddy 将自动调用 `planning-with-files` 并创建三个规划文件。
...
该技能在 `.codebuddy/skills/planning-with-files/references/` 中包含入门模板：
- `task_plan.md` — 阶段跟踪模板
- `findings.md` — 研究存储模板
- `progress.md` — 会话日志模板
...
CodeBuddy 调用 `planning-with-files` 并创建规划文件。
...
> “Markdown 是我在磁盘上的‘工作记忆’。由于我以迭代方式处理信息，并且我的活动上下文有限，Markdown 文件充当笔记的草稿纸、进度的检查点以及最终交付成果的构建块。”
> — Manus AI
...
模板位于：
- **工作区：** `.codebuddy/skills/planning-with-files/references/`
- **个人：** `~/.codebuddy/skills/planning-with-files/references/`
...
编辑 `.codebuddy/skills/planning-with-files/SKILL.md` 以自定义：
- 更改描述中的触发短语
- 调整规划模式
- 添加团队特定规则
...
- **CodeBuddy 文档：** https://www.codebuddy.ai/docs/ide/Features/Skills
